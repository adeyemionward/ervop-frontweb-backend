    ...
    protected function buildPrompt(string $page, string $section, array $businessInfo): string
{
        $templates = [
            ...
            'faq' => [
                'faq' => "Generate exactly 6 FAQ items for {$businessInfo['business_name']} in {$businessInfo['business_currency']} ,{$businessInfo['business_industry']}.
                        Business: {$businessInfo['business_description']}
     ...
     */
    protected function generateAndStoreSection($userId, $page, $section, $businessInfo)
    {
  ...
  protected function callDeepSeekApi(string $prompt, array $options = [])
{
    ...
    try {
        ...
        ->timeout(180) // ⏱️ Prevent hanging forever but long enough for Groq
        ->retry(4, 4000, function ($exception, $request) {
            // Retry on timeout or rate limit
            if ($exception instanceof \Illuminate\Http\Client\ConnectionException) {
                Log::warning("⚠️ DeepSeek connection timeout, retrying...");
                return true;
            }
            if ($exception->getCode() === 429) {
                Log::warning("⚠️ Rate limit hit (429), retrying after short delay...");
                sleep(10);
                return true;
            }
            return false;
        ...
        if ($response->failed()) {
            ...
            Log::error("❌ DeepSeek API failed with HTTP {$status}: {$body}");

            // Handle specific errors
            if ($status === 429) {
                Log::warning("🕒 Rate limit reached. Waiting before retry...");
                sleep(30);
                return $this->callDeepSeekApi($prompt, $options); // one retry
            }
        ...
        if (empty(trim($content))) {
            Log::warning("⚠️ DeepSeek returned empty response. Retrying...");
            sleep(5);
            return $this->callDeepSeekApi($prompt, $options); // one retry if empty
        }

        return $content;

    } catch (\Illuminate\Http\Client\ConnectionException $e) {
        Log::error("🚨 DeepSeek ConnectionException: " . $e->getMessage());
        sleep(10); // wait a bit before retry
        return $this->callDeepSeekApi($prompt, $options);
    } catch (\Throwable $e) {
